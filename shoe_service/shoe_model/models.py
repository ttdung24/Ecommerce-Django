...
class ShoeTag(models.Model):
    ...
    product = models.ForeignKey(
        Shoe, related_name="tags", on_delete=models.CASCADE, null=True)

    def __str__(self):
        return self.title


class ShoeThumnailImage(models.Model):
    alt = models.CharField(max_length=200)
    img = models.ImageField(null=True, blank=True,
                            upload_to="static/images/thumbnails")
    product = models.ForeignKey(
        Shoe, on_delete=models.CASCADE, related_name='thumbnail_images', null=True)
    @property
    def imageURL(self):
        try:
            url = self.img.url
...
class ShoeImages(models.Model):
    ...
    product = models.ForeignKey(
        Shoe, on_delete=models.CASCADE, related_name='images', null=True)
